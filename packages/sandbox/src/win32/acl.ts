...
/**
 * One lock file per protected path: `<GetTempPathW()>\pi-sandbox-dsh-acl-locks\<first 16
 * hex of sha256(lowercased path)>.lock`. The lock root derives from
 * GetTempPathW (never from runner argv or process env), and the lowercasing
...
export function lockFilePath(api: Win32Bindings, path: string): string {
  ...
  return join(getTempPath(api), 'pi-sandbox-dsh-acl-locks', `${digest}.lock`)
}
